...
class PlotManager:
    ...
    def pre_process_file(
        self, file_path: Path
    ) -> Tuple[Path, Optional[CacheEntry], Optional[stat_result], Optional[DiskProver]]:
        if not self._refreshing_enabled:
        ...
        return file_path, self.cache.get(file_path), stat_info, prover
        ...
        for path, cache_entry, stat_info, prover in pre_process_results:
            if stat_info is not None:
                if cache_entry is None:
                    ...
                    process_args.append((path, prover.get_memo()))
                    stat_infos[path] = stat_info
                    provers[path] = prover
                else:
                    new_plot_info = self.post_process_file(path, stat_info, cache_entry)
